...
export default function Maininfo() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetch("https://amaranocfirebasa-default-rtdb.firebaseio.com/dbimg.json");
                if (!res.ok) throw new Error("Network response was not ok");
                const parsed: MainData[] = await res.json();
                setData(parsed);
            } catch (err) {
                console.error(err);
            } finally {
                ...
                <div>
                    <div className="maininfodiv flex items-center justify-between rounded-2xl border p-3 shadow-sm">
                        ...
                        <div className="flex items-center space-x-2">
                            <span className="font-semibold text-orange-600">{Number(item.gin).toLocaleString()}</span>
                    ...
                    <div className="grid grid-cols-3 gap-3 mt-10">
                        <div className="col-span-2 row-span-2 relative rounded-xl overflow-hidden">
                            <img src={item.src} alt={item.tex} className="imgmain w-full object-cover rounded-xl" />
                            <span className="absolute top-5 left-5 text-7xl font-bold text-orange-500 drop-shadow-lg">15%</span>
                        </div>
                                    ...
                                    d ? (
                                        <button
                                            ...
                                            className={`p-2 rounded-full w-10 h-10 mx-auto transition 
                                                ${selectedDay === d ? "bg-orange-500 text-white shadow" : "hover:bg-orange-100"}`}
                                        >
                                            {d}
                                        </button>
                                    ) : <span key={i}></span>
                    ...
                    {/* Map & Images */}
                    <div className="aravelutyun mt-10">
                    ...
                    <iframe
                        ...
                        className="qartez w-full h-full border-0 mt-6"
                        loading="lazy"
                        allowFullScreen
                        referrerPolicy="no-referrer-when-downgrade"
                    ></iframe>
                    <div className="aravelutyun mt-6">
                        <img src="/imagekarciq.png" alt="Decoration" />
                    </div>

                    {/* Suggestions Section */}
                    <div className="vernagir mt-10 text-center">
                        <p className="m-3">—————————————————————————————————</p>
